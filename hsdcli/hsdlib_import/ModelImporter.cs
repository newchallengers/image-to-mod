...
using System.IO;
using System.Linq;
// ModelImportSettings, MeshImportSettings, MaterialImportSettings live in HSDRawViewer.Converters (same namespace as ModelImporter).

namespace HSDRawViewer.Converters
{
    /// <summary>
    /// Headless-Linux shim replacing HSDRawViewer.GUI's ProgressBarDisplay+BackgroundWorker.
    /// The ported Import() calls w.ReportProgress; we forward to stdout so it lands in the
    /// hsdcli output stream for /modder/process to log.
    /// </summary>
    public sealed class NullBackgroundWorker : BackgroundWorker {
        public NullBackgroundWorker() { WorkerReportsProgress = true; }
    }
    public abstract class ProgressClass {
        public string ProgressStatus { get; set; }
        public abstract void Work(BackgroundWorker w);
    }
    ...
    public class ModelImporter : ProgressClass
    {
        ...
        /// <summary>
        /// Headless entry — used by hsdcli splice-mesh. No dialogs, no
        /// WinForms. Takes a mesh file, returns a rebuilt HSD_JOBJ that
        /// the caller can substitute into the base .DAT root.
        /// </summary>
        public static HSD_JOBJ ImportModelFromFileHeadless(string f)
        {
            IOScene scene = IOManager.LoadScene(f, new ImportSettings() { Triangulate = true });
            if (scene.Models.Count == 0) return null;

            // Blender's dumb root bone strip
            for (int i = 0; i < scene.Models[0].Skeleton.RootBones.Count; i++) {
                if (scene.Models[0].Skeleton.RootBones[i].Name.Equals("Armature")) {
                    ...
                    while (joint != null) {
                        while (joint.Sibling != null && joint.Sibling.Type != BoneType.JOINT)
            ...
            var meshS = scene.Models[0].Meshes.Select(m => new MeshImportSettings(m)).ToList();
            var matS  = new List<MaterialImportSettings>();
            var imp = new ModelImporter(Path.GetDirectoryName(f), scene, scene.Models[0], settings, meshS, matS);
            var w = new NullBackgroundWorker();
            imp.Work(w);
            return imp.NewModel;
        }

        // Original WinForms overloads removed; ImportModelFromFileHeadless is the only entry point.
        private static HSD_JOBJ ImportModelFromFile_LEGACY(string f, HSD_JOBJ original = null)
        {
            if (f != null)
            {
                ...
                // WinForms preserve-skeleton dialog stripped for headless.
                return ImportModelFromFileHeadless(f);
            }
            return null;
        }

        // ImportHSDM stripped (relied on HsdImportHelper + WinForms).

        // ImportModelFromFile(HSD_JOBJ) + ReplaceModelFromFile stripped — WinForms file dialog. Use headless path instead.
        ...
        /// <param name="from"></param>
        /// <param name="to"></param>
        // ReplaceWithBonesFromFile(HSD_JOBJ, HSD_JOBJ) stripped (relied on LiveJObj GUI rendering type).
        // The recursive IOBone variant above is enough for our headless flow.
        ...
        public override void Work(BackgroundWorker w)
        {
            try { Import(w); }
                ...
                Console.Error.WriteLine("ModelImporter FAILED: " + e);
                throw;
            }
        }
...
#if DEBUG
            //
            // JOBJExtensions.MergeIntoOneObject dropped — ImageSharp/rendering dep. Not needed for the modder flow.
#endif
        ...
        private void ProcessMesh(IOScene scene, IOMesh mesh, HSD_JOBJ rootnode, ref POBJ_Generator POBJGen)
        {
            ...
            // BUMP-map path skipped — CalculateTangentBitangent lives in a IONET
            // extension we don't wire in. The mod flow generates unshaded skins.
            ...
            foreach (IOPolygon poly in mesh.Polygons)
            {
                ...
                foreach (int face in poly.Indicies)
                {
                    ...
                    // set final vertex data (inlined GXTranslator.fromVector3)
                    { var _tk = tkvert;             vertex.POS   = new GXVector3(_tk.X, _tk.Y, _tk.Z); }
                    { var _tk = tknrm.Normalized(); vertex.NRM   = new GXVector3(_tk.X, _tk.Y, _tk.Z); }
                    { var _tk = tktan;              vertex.TAN   = new GXVector3(_tk.X, _tk.Y, _tk.Z); }
                    { var _tk = tkbitan;            vertex.BITAN = new GXVector3(_tk.X, _tk.Y, _tk.Z); }
            ...
            if (settings.ImportTexture)
            {
                if (material.DiffuseMap != null && !string.IsNullOrEmpty(material.DiffuseMap.FilePath))
                {
                    ...
                    if (File.Exists(texturePath) && (texturePath.ToLower().EndsWith(".png") || texturePath.ToLower().EndsWith(".bmp")))
                    {
                        ...
                        {
                            /* Texture PNG→TPL encoding (ImportImage + IsTransparent) lives in
                               TOBJExtensions in HSDRawViewer that pull ImageSharp. Not wired
                               into the modder flow — the character's Meshy textures come in
                               via a later PNG→TPL step or via HSDRaw manually. Skip this
                               branch; the resulting mesh will use the base-DAT texture until
                               textures are wired in. */
                        }
                    }
                }
            }
